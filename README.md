...
See [DEPLOYMENT.md](./DEPLOYMENT.md) for complete setup instructions (including **Vercel**, **local builds**, and **npm package publishing** / deprecation of `@ekkos/mcp-server`).

### MCP & npm messaging

- **Primary:** `@ekkos/cli` — users run **`npx -y @ekkos/cli@latest mcp`** (or `ekkos mcp` globally).
- **Deprecated:** `@ekkos/mcp-server` — legacy shim only; docs and examples should not recommend it for new installs.
