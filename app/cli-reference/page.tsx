...
const runningCommands: Command[] = [
  ...
  },
  {
    name: 'ekkos cursor',
    description:
      'Launch the Cursor Agent CLI with ekkOS memory. Deploys an MCP server config and Golden Loop rules into your workspace, then spawns the agent binary with session tracking. The Cursor agent gets access to all ekkOS tools (search, forge, track, outcome) via MCP.',
    usage: 'ekkos cursor [options]',
    flags: [
      { flag: '-s, --session <name>', desc: 'Session name (auto-generated if omitted)' },
      { flag: '-m, --model <model>', desc: 'Model to use (e.g., claude-4.6-opus-max)' },
      { flag: '--mode <mode>', desc: 'Agent mode: plan or ask' },
      { flag: '--plan', desc: 'Start in plan mode' },
      { flag: '--cloud', desc: 'Start in cloud mode' },
      { flag: '-p, --print', desc: 'Non-interactive headless mode' },
      { flag: '--output-format <format>', desc: 'Output format for print mode: text | json | stream-json' },
      { flag: '--yolo', desc: 'Force allow all commands' },
      { flag: '--sandbox <mode>', desc: 'Sandbox mode: enabled or disabled' },
      { flag: '--dashboard', desc: 'Launch with tmux dashboard split' },
      { flag: '--continue', desc: 'Continue previous Cursor session' },
      { flag: '--resume [chatId]', desc: 'Resume specific Cursor session' },
      { flag: '--setup-only', desc: 'Deploy MCP + rules without launching the agent' },
      { flag: '-v, --verbose', desc: 'Show debug output' },
    ],
    icon: Terminal,
    example: `$ ekkos cursor

  🧠 ekkOS_ + Cursor Agent
  📂 Session: bright-falcon-soars
  📦 MCP: .cursor/mcp.json
  📜 Rules: .cursor/rules/ekkos-memory.mdc
  🧠 ekkOS_Pulse  Loaded!
